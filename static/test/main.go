...
func login(w http.ResponseWriter, r *http.Request) {
	...
	if name != "" && pass != "" {
		if b, uuid := userExists(u); b == true {
			setSession(&User{Username: name, Uuid: uuid}, w)
			redirect = "/example"
		} else {
			setMsg(w, "message", "please signup or enter a valid username and password!")
		}
	} else {
		setMsg(w, "message", "The Username or Password fields are empty!")
	}
...
func signup(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		if n := checkUser(r.FormValue("userName")); n == true {
			setMsg(w, "username", "User already exists. Please enter a unique name!")
		...
		if r.FormValue("password") != r.FormValue("cpassword") {
			setMsg(w, "password", "The passwords do not match!")
			http.Redirect(w, r, "/signup", 302)
			return
		}
		if result == true {
